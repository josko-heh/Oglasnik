...
public class PrivatniKorisnikController {
	...
	public void initialize() {
		...
		try {
			privatniKorisniciTable.setItems(FXCollections.observableArrayList(BazaPodataka.dohvatiKorisnikePremaKriterijima(new PrivatniKorisnik(0, "", "", "", ""))));
...
//-----------prikazi funkcije-----------
	...
	@FXML private void pretraziGumb() {
		...
		try {
			privatniKorisniciTable.setItems(FXCollections.observableArrayList(BazaPodataka.dohvatiKorisnikePremaKriterijima(new PrivatniKorisnik(0, email, telefon, ime, prezime))));
		} catch (BazaPodatakaException e) {
			e.prikaziDijalog(e.getMessage());
			e.printStackTrace();
		}	
